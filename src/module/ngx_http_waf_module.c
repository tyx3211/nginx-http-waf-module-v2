...
static ngx_int_t ngx_http_waf_collect_request_body(ngx_http_request_t *r, ngx_str_t *body_str)
{
    ngx_chain_t *cl;
    ngx_buf_t   *b;
    size_t       len = 0;
...
static waf_rc_e waf_stage_detect_bundle(ngx_http_request_t* r,
                                        ...
                                        ngx_http_waf_ctx_t*       ctx)
{
    /* 最小实现：遍历 detect 段 buckets，支持 URI/ARGS_COMBINED/ARGS_NAME/ARGS_VALUE/HEADER 的 CONTAINS/REGEX 匹配 */
    ...
    for (ngx_uint_t target = 0; target <= WAF_T_HEADER; target++) {
        ...
        if (bucket == NULL || bucket->nelts == 0) continue;
        ...
        for (ngx_uint_t i = 0; i < bucket->nelts; i++) {
            ...
            switch (rule->target) {
                    ...
                    if (rule->match == WAF_MATCH_CONTAINS) {
                        ...
                        if (pats) {
                            for (ngx_uint_t k = 0; k < rule->patterns->nelts; k++) {
                                if (ngx_http_waf_contains_ci(&subj, &pats[k], rule->caseless)) { matched = 1; break; }
                            }
                        }
                ...
                case WAF_T_HEADER: {
                    ...
                    if (ngx_http_waf_get_header(r, &rule->header_name, &hv)) {
                        if (rule->match == WAF_MATCH_CONTAINS) {
                            ...
                            if (pats) {
                                for (ngx_uint_t k = 0; k < rule->patterns->nelts; k++) {
                                    if (ngx_http_waf_contains_ci(&hv, &pats[k], rule->caseless)) { matched = 1; break; }
                                }
                            }
                        } else if (rule->match == WAF_MATCH_REGEX) {
                ...
                case WAF_T_BODY: {
                    ...
                    if (rule->match == WAF_MATCH_CONTAINS) {
                        ...
                        if (pats) {
                            for (ngx_uint_t k = 0; k < rule->patterns->nelts; k++) {
                                if (ngx_http_waf_contains_ci(&body, &pats[k], rule->caseless)) { matched = 1; break; }
                            }
                        }
                    } else if (rule->match == WAF_MATCH_REGEX) {
                        matched = ngx_http_waf_regex_any_match(rule->compiled_regexes, &body);
                    }
                    break; 
                }
            ...
            switch (rule->action) {
                ...
                case WAF_ACT_LOG:
                default: {
                    waf_enforce_log(r, mcf, lcf, ctx, rule->id, (ngx_uint_t)(rule->score > 0 ? rule->score : 0));
                    /* 继续遍历后续规则 */
                    break; 
                }
            }
...
static ngx_int_t ngx_http_waf_access_handler(ngx_http_request_t *r)
{
    ...
    ngx_int_t rc = ngx_http_read_client_request_body(r, (ngx_http_client_body_handler_pt) ngx_http_waf_post_read_body_handler);
