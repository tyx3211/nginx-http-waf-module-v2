...
static void waf_record_event(ngx_http_request_t *r, ngx_http_waf_ctx_t *ctx,
                             waf_intent_e intent, ngx_uint_t rule_id_or_0,
                             ngx_uint_t score_delta)
{
  if (ctx == NULL)
    return;
  /* 存根：仅提升日志级别，记录为 INFO（命中）或 DEBUG（普通） */
  waf_log_append_event(
      r, ctx, (intent == WAF_INTENT_BLOCK) ? WAF_LOG_INFO : WAF_LOG_DEBUG);
  (void)rule_id_or_0;
  if (score_delta > 0) {
...
{
  (void)waf_enforce(r, mcf, lcf, ctx, WAF_INTENT_LOG, NGX_DECLINED, 0,
                    score_delta);
  /* 存根阶段不触发封禁，直接继续 */
  return WAF_RC_CONTINUE;
}
