 ...
 * M2：编译期快照最小实现
 *
 * 目标：将 M1 产出的合并文档（只读 yyjson_doc）编译为可被运行期直接消费的只读快照。
 * 本文件实现最小可用集：
...
/* ------------------------ 工具：字符串复制 ------------------------ */
static ngx_int_t
waf_copy_str(ngx_pool_t* pool, const char* s, size_t len, ngx_str_t* out)
{
    if (s == NULL || len == 0) {
        out->data = NULL;
        out->len = 0;
...
{
    if (len == 8 && ngx_strncasecmp((u_char*)s, (u_char*)"CONTAINS", 8) == 0) { *out = WAF_MATCH_CONTAINS; return NGX_OK; }
    if (len == 5 && ngx_strncasecmp((u_char*)s, (u_char*)"REGEX", 5) == 0) { *out = WAF_MATCH_REGEX; return NGX_OK; }
    if (len == 4 && ngx_strncasecmp((u_char*)s, (u_char*)"CIDR", 4) == 0) { *out = WAF_MATCH_CIDR; return NGX_OK; }
    return NGX_ERROR;
}

static ngx_int_t waf_parse_action(const char* s, size_t len, waf_action_e* out)
{
    if (len == 4 && ngx_strncasecmp((u_char*)s, (u_char*)"DENY", 4) == 0) { *out = WAF_ACT_DENY; return NGX_OK; }
    if (len == 3 && ngx_strncasecmp((u_char*)s, (u_char*)"LOG", 3) == 0) { *out = WAF_ACT_LOG; return NGX_OK; }
    if (len == 6 && ngx_strncasecmp((u_char*)s, (u_char*)"BYPASS", 6) == 0) { *out = WAF_ACT_BYPASS; return NGX_OK; }
    return NGX_ERROR;
}
...
static ngx_int_t waf_parse_target(const char* s, size_t len, waf_target_e* out)
{
    ...
    if (len == 9 && ngx_strncasecmp((u_char*)s, (u_char*)"CLIENT_IP", 9) == 0) { *out = WAF_T_CLIENT_IP; return NGX_OK; }
    if (len == 3 && ngx_strncasecmp((u_char*)s, (u_char*)"URI", 3) == 0) { *out = WAF_T_URI; return NGX_OK; }
    /* 注意："ARGS_COMBINED" 长度为 13 */
    if (len == 13 && ngx_strncasecmp((u_char*)s, (u_char*)"ARGS_COMBINED", 13) == 0) { *out = WAF_T_ARGS_COMBINED; return NGX_OK; }
    if (len == 9 && ngx_strncasecmp((u_char*)s, (u_char*)"ARGS_NAME", 9) == 0) { *out = WAF_T_ARGS_NAME; return NGX_OK; }
    if (len == 10 && ngx_strncasecmp((u_char*)s, (u_char*)"ARGS_VALUE", 10) == 0) { *out = WAF_T_ARGS_VALUE; return NGX_OK; }
    if (len == 4 && ngx_strncasecmp((u_char*)s, (u_char*)"BODY", 4) == 0) { *out = WAF_T_BODY; return NGX_OK; }
    if (len == 6 && ngx_strncasecmp((u_char*)s, (u_char*)"HEADER", 6) == 0) { *out = WAF_T_HEADER; return NGX_OK; }
    return NGX_ERROR;
}

/* ------------------------ 工具：tags 数组复制 ------------------------ */
static ngx_int_t
waf_copy_tags(ngx_pool_t* pool, yyjson_val* tags_node, ngx_array_t** out_tags)
{
    if (tags_node == NULL) { *out_tags = NULL; return NGX_OK; }
    if (!yyjson_is_arr(tags_node)) return NGX_ERROR;
    size_t n = yyjson_arr_size(tags_node);
    ngx_array_t* arr = ngx_array_create(pool, n > 0 ? n : 1, sizeof(ngx_str_t));
    if (arr == NULL) return NGX_ERROR;
    for (size_t i = 0; i < n; i++) {
        yyjson_val* it = yyjson_arr_get(tags_node, i);
        if (!yyjson_is_str(it)) return NGX_ERROR;
        const char* s = yyjson_get_str(it);
        size_t len = yyjson_get_len(it);
        ngx_str_t* slot = ngx_array_push(arr);
        if (slot == NULL) return NGX_ERROR;
        if (waf_copy_str(pool, s, len, slot) != NGX_OK) return NGX_ERROR;
    }
    *out_tags = arr;
    return NGX_OK;
}
...
static ngx_int_t
waf_copy_patterns(ngx_pool_t* pool, yyjson_val* pattern_node, ngx_array_t** out_patterns)
{
...
static ngx_int_t waf_infer_phase(const waf_compiled_rule_t* r, ngx_log_t* log, ngx_http_waf_json_error_t* err)
{
    (void)log;
    /* 若 JSON 显式存在 phase（M2 最小实现暂未读取），此处可覆盖。当前按 target+action 推断：
     * - CLIENT_IP + BYPASS => IP_ALLOW
     * - CLIENT_IP + DENY   => IP_BLOCK
     * - URI + BYPASS       => URI_ALLOW
     * - 其他                => DETECT
     */
    if (r->target == WAF_T_CLIENT_IP && r->action == WAF_ACT_BYPASS) return (r->phase == 0 ? (waf_phase_e)WAF_PHASE_IP_ALLOW : r->phase);
    if (r->target == WAF_T_CLIENT_IP && r->action == WAF_ACT_DENY)   return (r->phase == 0 ? (waf_phase_e)WAF_PHASE_IP_BLOCK : r->phase);
    if (r->target == WAF_T_URI       && r->action == WAF_ACT_BYPASS) return (r->phase == 0 ? (waf_phase_e)WAF_PHASE_URI_ALLOW : r->phase);
...
{
    if (len == 8 && ngx_strncasecmp((u_char*)s, (u_char*)"ip_allow", 8) == 0) { *out = WAF_PHASE_IP_ALLOW; return NGX_OK; }
    if (len == 8 && ngx_strncasecmp((u_char*)s, (u_char*)"ip_block", 8) == 0) { *out = WAF_PHASE_IP_BLOCK; return NGX_OK; }
    if (len == 9 && ngx_strncasecmp((u_char*)s, (u_char*)"uri_allow", 9) == 0) { *out = WAF_PHASE_URI_ALLOW; return NGX_OK; }
    if (len == 6 && ngx_strncasecmp((u_char*)s, (u_char*)"detect", 6) == 0) { *out = WAF_PHASE_DETECT; return NGX_OK; }
    return NGX_ERROR;
}

static ngx_int_t waf_validate_phase_combo(waf_phase_e phase, waf_target_e target, waf_action_e action)
{
    switch (phase) {
...
{
    if (bucket == NULL || bucket->nelts <= 1) return;
...
/* ------------------------ 预编译：REGEX/CIDR ------------------------ */
static ngx_int_t waf_precompile_regexes(ngx_pool_t* pool,
                                        ngx_log_t* log,
                                        waf_compiled_rule_t* rule)
{
    if (rule->match != WAF_MATCH_REGEX) return NGX_OK;
    ngx_uint_t n = rule->patterns ? rule->patterns->nelts : 0;
    if (n == 0) return NGX_OK;

    rule->compiled_regexes = ngx_array_create(pool, n, sizeof(ngx_regex_t*));
    if (rule->compiled_regexes == NULL) return NGX_ERROR;
...
static ngx_int_t waf_precompile_cidrs(ngx_pool_t* pool,
                                       ngx_log_t* log,
                                       waf_compiled_rule_t* rule)
{
    (void)log;
    if (rule->match != WAF_MATCH_CIDR) return NGX_OK;
    ngx_uint_t n = rule->patterns ? rule->patterns->nelts : 0;
    if (n == 0) return NGX_OK;

    rule->compiled_cidrs = ngx_array_create(pool, n, sizeof(ngx_cidr_t));
    if (rule->compiled_cidrs == NULL) return NGX_ERROR;
...
/* ------------------------ 主编译入口 ------------------------ */
ngx_int_t ngx_http_waf_compile_rules(ngx_pool_t* pool,
                                     ngx_log_t* log,
                                     ...
                                     ngx_http_waf_json_error_t* err)
{
    ...
    *out = NULL;
    ...
    for (size_t i = 0; i < rn; i++) {
        ...
        if (!id_node || !yyjson_is_int(id_node) || !target_node || !match_node || !pattern_node || !action_node) {
            if (err) {
                err->file.len = 0; err->file.data = NULL;
                err->json_pointer.len = 0; err->json_pointer.data = NULL;
        ...
        /* ID 唯一性：编译期再次校验，避免残余重复 */
        {
            ...
            if (found != NULL) {
                if (err) { ngx_str_set(&err->message, "检测到重复的规则 id"); }
        ...
        if (yyjson_is_arr(target_node)) {
            ...
            if (tn == 0) {
                if (err) { ngx_str_set(&err->message, "target 数组不能为空"); }
                return NGX_ERROR;
            }
            for (size_t ti = 0; ti < tn; ti++) {
                yyjson_val* t = yyjson_arr_get(target_node, ti);
                if (!yyjson_is_str(t)) {
                    if (err) { ngx_str_set(&err->message, "target 数组元素必须为字符串"); }
                    return NGX_ERROR;
                }
                ...
                if (waf_parse_target(ts, tl, &tcode) != NGX_OK) {
                    if (err) { ngx_str_set(&err->message, "未知的 target 值"); }
                    return NGX_ERROR;
                }
                ...
                if (tcode == WAF_T_HEADER) {
                    ...
                    if (!hn || !yyjson_is_str(hn) || yyjson_get_len(hn) == 0) {
                        if (err) { ngx_str_set(&err->message, "HEADER 目标必须提供非空 headerName"); }
                        return NGX_ERROR;
                    }
                    if (waf_copy_str(pool, yyjson_get_str(hn), yyjson_get_len(hn), &tmp.header_name) != NGX_OK) return NGX_ERROR;
                }
                ...
                /* match */
                {
                    ...
                    if (waf_parse_match(ms, ml, &tmp.match) != NGX_OK) {
                        if (err) { ngx_str_set(&err->message, "非法 match 值"); }
                        return NGX_ERROR;
                    }
                }
                ...
                if (waf_copy_patterns(pool, pattern_node, &tmp.patterns) != NGX_OK) {
                    if (err) { ngx_str_set(&err->message, "pattern 非法或为空"); }
                    return NGX_ERROR;
                }
                ...
                /* action */
                {
                    ...
                    if (waf_parse_action(as, al, &tmp.action) != NGX_OK) {
                        if (err) { ngx_str_set(&err->message, "非法 action 值"); }
                        return NGX_ERROR;
                    }
                }
                ...
                if (waf_copy_tags(pool, yyjson_obj_get(r, "tags"), &tmp.tags) != NGX_OK) {
                    if (err) { ngx_str_set(&err->message, "tags 必须为字符串数组"); }
                    return NGX_ERROR;
                }
                ...
                /* phase：显式覆盖或推断，并校验组合 */
                {
                    ...
                    if (ph && yyjson_is_str(ph)) {
                        ...
                        if (waf_parse_phase_text_ci(yyjson_get_str(ph), yyjson_get_len(ph), &phv) != NGX_OK) {
                            if (err) { ngx_str_set(&err->message, "phase 取值非法"); }
                            return NGX_ERROR;
                        }
                        tmp.phase = phv;
                    } else {
                    ...
                    if (waf_validate_phase_combo(tmp.phase, tmp.target, tmp.action) != NGX_OK) {
                        if (err) { ngx_str_set(&err->message, "phase/target/action 组合非法"); }
                        return NGX_ERROR;
                    }
                }
                ...
                *slot = tmp;
                ...
                /* 分桶索引：保存指针 */
                if (waf_bucket_append(pool, snap, slot->phase, slot->target, slot) != NGX_OK) return NGX_ERROR;

                /* 预编译 REGEX/CIDR */
                if (waf_precompile_regexes(pool, log, slot) != NGX_OK) return NGX_ERROR;
                if (waf_precompile_cidrs(pool, log, slot) != NGX_OK) return NGX_ERROR;
            }
            ...
            if (rule.target == WAF_T_HEADER) {
                ...
                if (waf_copy_str(pool, yyjson_get_str(hn), yyjson_get_len(hn), &rule.header_name) != NGX_OK) return NGX_ERROR;
            }
            ...
            *slot = rule;
            ...
            if (err) { ngx_str_set(&err->message, "target 必须为字符串或字符串数组"); }
            return NGX_ERROR;
        }
    }
