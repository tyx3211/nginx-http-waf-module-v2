...
static void
waf_json_reset_error(ngx_http_waf_json_error_t* err)
{
    if (err == NULL) {
        return;
    }
 ...
 */
static ngx_int_t
waf_json_set_error(waf_merge_ctx_t* ctx,
 ...
 */
static ngx_int_t
waf_str_copy(ngx_pool_t* pool, const ngx_str_t* src, ngx_str_t* dst)
{
 ...
 */
static ngx_int_t
waf_pointer_concat(ngx_pool_t* pool, const char* base, const char* suffix, ngx_str_t* out)
{
    ...
    if (bl) ngx_memcpy(p, base, bl);
    if (sl) ngx_memcpy(p + bl, suffix, sl);
    p[len] = '\0';
    out->data = p;
    out->len = len;
    return NGX_OK;
 ...
 */
static ngx_int_t
ngx_http_waf_str_eq(const ngx_str_t* a, const ngx_str_t* b)
{
    if (a == NULL || b == NULL) {
        return 0;
    }
 ...
 */
static ngx_int_t
ngx_http_waf_push_path(ngx_array_t* stack, const ngx_str_t* path)
{
 ...
 */
static ngx_uint_t
ngx_http_waf_path_in_stack(ngx_array_t* stack, const ngx_str_t* path)
{
 ...
 */
static ngx_int_t
ngx_http_waf_normalize_path(ngx_pool_t* pool, ngx_str_t* path)
{
 ...
 */
ngx_int_t
ngx_http_waf_dirname(ngx_pool_t* pool, const ngx_str_t* path, ngx_str_t* out_dir)
{
 ...
 */
static ngx_int_t
ngx_http_waf_join_path_internal(ngx_pool_t* pool,
 ...
 */
ngx_int_t
ngx_http_waf_join_path(ngx_pool_t* pool,
                       const ngx_str_t* base_dir,
                       const ngx_str_t* path,
                       ngx_str_t* out_abs)
{
 ...
 */
static yyjson_doc*
ngx_http_waf_json_read_single(ngx_pool_t* pool,
                              ngx_log_t* log,
 ...
 * 作用: 解析 extends 引用：按绝对/相对/jsons_root 解析 -> 展开 Nginx 前缀 -> 规范化
 */
static ngx_int_t
ngx_http_waf_resolve_path(ngx_pool_t* pool,
                          ngx_log_t* log,
 ...
 */
static ngx_int_t
waf_target_code_from_string(const char* s, size_t len, waf_target_e* out)
{
 ...
 */
static ngx_uint_t
waf_target_list_contains(const waf_target_list_t* list, waf_target_e code)
{
 ...
 */
static ngx_int_t
waf_target_list_add(waf_target_list_t* list, waf_target_e code)
{
 ...
 */
static ngx_int_t
waf_target_list_expand_and_add(waf_target_list_t* list, waf_target_e code)
{
    if (code == WAF_TARGET_ALL_PARAMS) {
        if (waf_target_list_add(list, WAF_TARGET_URI) != NGX_OK) return NGX_ERROR;
        if (waf_target_list_add(list, WAF_TARGET_ARGS_COMBINED) != NGX_OK) return NGX_ERROR;
        if (waf_target_list_add(list, WAF_TARGET_BODY) != NGX_OK) return NGX_ERROR;
        return NGX_OK;
    }
 ...
 */
static ngx_int_t
waf_parse_target_value(waf_merge_ctx_t* ctx,
                       yyjson_val* node,
                       const ngx_str_t* file,
                       const char* pointer,
...
waf_build_target_mut_value(waf_merge_ctx_t* ctx, const waf_target_list_t* list)
{
    if (list->count == 0) {
        return NULL;
    }
 ...
 */
static ngx_int_t
waf_assign_target_to_rule(waf_merge_ctx_t* ctx,
                          yyjson_mut_val* rule,
 ...
 */
static ngx_uint_t
waf_string_equals_ci(const char* s, size_t len, const char* target)
{
 ...
 */
static ngx_int_t
waf_match_validate(const char* s, size_t len)
{
 ...
 */
static ngx_int_t
waf_action_validate(const char* s, size_t len)
{
 ...
 */
static ngx_int_t
waf_phase_validate(const char* s, size_t len)
{
 ...
 */
static ngx_int_t
waf_copy_tags_array(waf_merge_ctx_t* ctx,
                    yyjson_val* src,
 ...
 */
static ngx_uint_t
waf_rule_has_tag(yyjson_mut_val* rule, const char* tag)
{
 ...
 */
static ngx_uint_t
waf_rule_match_disable_id(yyjson_val* disable_ids, int64_t id)
{
 ...
 */
static ngx_uint_t
waf_rule_match_disable_tag(yyjson_val* disable_tags, yyjson_mut_val* rule)
{
 ...
 */
static ngx_int_t
waf_validate_additional_properties(waf_merge_ctx_t* ctx,
                                   yyjson_val* rule,
                                   const ngx_str_t* file,
                                   const char* pointer)
{
 ...
 */
static ngx_int_t
waf_parse_rule(waf_merge_ctx_t* ctx,
               yyjson_val* src_rule,
               waf_rule_entry_t* out,
               const ngx_str_t* file,
               const char* base_pointer)
{
    ...
    if (waf_copy_tags_array(ctx, yyjson_obj_get(src_rule, "tags"), rule_mut, file, base_pointer) != NGX_OK) {
        return NGX_ERROR;
    }
    ...
    if (caseless_node) {
        ...
        yyjson_mut_val* v = yyjson_is_true(caseless_node) ? yyjson_mut_true(ctx->out_doc) : yyjson_mut_false(ctx->out_doc);
        if (!k || !v || !yyjson_mut_obj_add(rule_mut, k, v)) {
 ...
 */
static waf_dup_policy_e
waf_parse_duplicate_policy(yyjson_val* root)
{
 ...
 */
static ngx_int_t
waf_parse_target_list_from_val(waf_merge_ctx_t* ctx,
                               yyjson_val* node,
                               const ngx_str_t* file,
                               const char* pointer,
                               waf_target_list_t* out)
 ...
 */
static ngx_int_t
waf_parse_rewrite_plan(waf_merge_ctx_t* ctx,
                       yyjson_val* plan_obj,
                       const ngx_str_t* file,
                       const char* base_pointer,
                       waf_rewrite_plan_t* plan)
{
    ...
    if (tag_map) {
        ...
        while ((key = yyjson_obj_iter_next(&it))) {
            ...
            if (waf_pointer_concat(ctx->pool, base_pointer, "/rewriteTargetsForTag", &rule->pointer) != NGX_OK) {
                return waf_json_set_error(ctx, file, base_pointer, "内存不足");
            }
    ...
    if (id_arr) {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (!ids || !yyjson_is_arr(ids) || !target) {
                return waf_json_set_error(ctx, file, base_pointer, "rewriteTargetsForIds 对象需包含 ids 数组与 target");
            }
            ...
            for (size_t j = 0; j < m; j++) {
                ...
                if (!yyjson_is_int(it)) {
                    return waf_json_set_error(ctx, file, base_pointer, "rewriteTargetsForIds.ids 元素必须为整数");
                }
            ...
            if (waf_parse_target_list_from_val(ctx, target, file, base_pointer, &rule->targets) != NGX_OK) {
                return NGX_ERROR;
            }
            if (waf_pointer_concat(ctx->pool, base_pointer, "/rewriteTargetsForIds", &rule->pointer) != NGX_OK) {
                return waf_json_set_error(ctx, file, base_pointer, "内存不足");
            }
        }
    }
...
static ngx_int_t
waf_apply_rewrite_plan(waf_merge_ctx_t* ctx,
                       waf_rewrite_plan_t* plan,
                       ngx_array_t* rules)
{
        ...
        for (ngx_uint_t i = 0; i < tn; i++) {
            ...
            for (ngx_uint_t j = 0; j < rule_count; j++) {
                ...
                if (waf_rule_has_tag(rule, (const char*)r->tag.data)) {
                    if (waf_assign_target_to_rule(ctx, rule, &r->targets, &entries[j].file, (const char*)r->pointer.data) != NGX_OK) {
                        return NGX_ERROR;
                    }
                }
            }
            ...
            for (ngx_uint_t j = 0; j < rule_count; j++) {
                for (ngx_uint_t k = 0; k < idn; k++) {
                    if (entries[j].id == id_list[k]) {
                        if (waf_assign_target_to_rule(ctx, entries[j].rule, &r->targets, &entries[j].file, (const char*)r->pointer.data) != NGX_OK) {
                            return NGX_ERROR;
                        }
                        break;
                    }
 ...
 */
static ngx_int_t
waf_merge_append_array(ngx_array_t* dest, ngx_array_t* src)
{
    if (!src || src->nelts == 0) {
        return NGX_OK;
    }
...
static ngx_int_t
waf_collect_rules(waf_merge_ctx_t* ctx,
                  const ngx_str_t* abs_path,
                  ngx_uint_t depth,
                  ...
                  ngx_array_t** out_rules)
{
    ...
    if (ngx_http_waf_path_in_stack(ctx->stack, abs_path)) {
        /* 与测试脚本关键字对齐（同时保留中文语义） */
        return waf_json_set_error(ctx, abs_path, NULL, "extends cycle detected | 检测到 extends 循环引用");
    }
    ...
    if (doc == NULL) {
        ctx->stack->nelts--;
        return waf_json_set_error(ctx, abs_path, (const char*)reader_err.json_pointer.data,
                                  "%V", &reader_err.message);
    }
        ...
        for (size_t i = 0; i < en; i++) {
            ...
            if (yyjson_is_str(item)) {
                ...
                if (!file_node || !yyjson_is_str(file_node)) {
                    yyjson_doc_free(doc);
                    ctx->stack->nelts--;
                    return waf_json_set_error(ctx, abs_path, "/meta/extends[]", "extends 对象必须包含 file 字段");
                }
            ...
            ngx_str_t child_abs;
            if (ngx_http_waf_resolve_path(ctx->pool, ctx->log, &ctx->jsons_root, &current_dir,
 ...
 */
yyjson_doc*
ngx_http_waf_json_load_and_merge(ngx_pool_t* pool,
                                             ngx_log_t* log,
                                             const ngx_str_t* base_dir,
                                             ...
                                             ngx_http_waf_json_error_t* err)
{
    ...
    ngx_str_t abs;
    if (ngx_http_waf_resolve_path(pool, log, &ctx.jsons_root, base_dir, entry_path, &abs, err) != NGX_OK) {
